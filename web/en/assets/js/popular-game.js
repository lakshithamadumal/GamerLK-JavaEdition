document.addEventListener("DOMContentLoaded", async function () {
    ...
    const response = await fetch("../LoadPopularGmames");
    ...
    if (response.ok) {
        ...
        // Only show first 4 games
        let gamesToShow = json.productList.slice(0, 4);

        let groupDiv = document.createElement("div");
        groupDiv.className = "game-list";

        for (let gamelist of gamesToShow) {
